...
//defined( '__LFB__' ) or die( 'Доступ запрещен' );
...
class database {
    ...
    */
    function database( $host='localhost', $user, $pass, $db='', $table_prefix='', $goOffline=true ) {
        ...
        @mysql_query("SET SESSION collation_connection = 'utf8_general_ci';", $this->_resource);
...
//exit();
    ...
    */
    function stderr( $showSQL = false ) {
        return "Функция БД завершилась ошибкой с номером $this->_errorNum"
...
//echo 'D: ' . $this->_db->getQuery();
    ...
    */
    function updateOrder( $where='' ) {
        ...
        if (!array_key_exists( 'ordering', get_class_vars( strtolower(get_class( $this )) ) )) {
            $this->_error = "ПРЕДУПРЕЖДЕНИЕ: ".strtolower(get_class( $this ))." не поддерживает упорядочивание.";
            return false;
        }
    ...
    */
    ...
    function checkout( $user_id, $oid=null ) {
        if (!array_key_exists( 'checked_out', get_class_vars( strtolower(get_class( $this )) ) )) {
            $this->_error = "ПРЕДУПРЕЖДЕНИЕ: ".strtolower(get_class( $this ))." не поддерживает разблокировки.";
            return false;
        }
        $k = $this->_tbl_key;
        if ($oid !== null) {
    ...
    function checkin( $oid=null ) {
        if (!array_key_exists( 'checked_out', get_class_vars( strtolower(get_class( $this )) ) )) {
            $this->_error = "ПРЕДУПРЕЖДЕНИЕ: ".strtolower(get_class( $this ))." не поддерживает блокировку.";
            return false;
        }
    ...
    function hit( $oid=null ) {
        global $bConfig_enable_log_items;
    ...
    */
    ...
    function publish( $cid=null, $publish=1, $user_id=0 ) {
        ...
        if (count( $cid ) < 1) {
            $this->_error = "Элементы не выбраны.";
            return false;
        }
